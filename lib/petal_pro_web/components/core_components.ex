  ...
  def layout(assigns) do
    ...
    <%= case @type do %>
      ...
      <% "public" -> %>
        <.public_layout
          {assigns}
          twitter_url={PetalPro.config(:twitter_url)}
          github_url={PetalPro.config(:github_url)}
          discord_url={PetalPro.config(:discord_url)}
          copyright_text={PetalPro.config(:business_name) <> ". All rights reserved."}
        >
          <:logo>
            <.logo class="h-10" />
          </:logo>
