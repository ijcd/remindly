defmodule RemindlyWeb.OrgTeamLive do
  use RemindlyWeb, :live_view
  alias Remindly.Orgs
  alias Remindly.Repo

  import RemindlyWeb.OrgSettingsLayoutComponent

  on_mount {RemindlyWeb.OrgOnMountHooks, :require_org_admin}
  ...
  def handle_event("delete_member", %{"id" => id}, socket) do
    ...
    case Orgs.delete_membership(membership) do
      {:ok, membership} ->
        Remindly.Logs.log("orgs.delete_member", %{
        ...
        if membership.user_id == socket.assigns.current_user.id do
          {:noreply,
           ...
           |> push_navigate(to: RemindlyWeb.Helpers.home_path(socket.assigns.current_user))}
        else
          RemindlyWeb.UserAuth.disconnect_user_liveviews(member_user)
  ...
  def handle_event("delete_invitation", %{"id" => id}, socket) do
    ...
    Remindly.Logs.log("orgs.delete_invitation", %{
      user: socket.assigns.current_user,
      org_id: org.id,
      target_user_id: nil,
      metadata: %{
