defmodule RemindlyWeb.UserTOTPController do
  use RemindlyWeb, :controller

  alias Remindly.Accounts
  alias RemindlyWeb.UserAuth
  ...
  def create(conn, %{"user" => user_params}) do
    ...
    case Accounts.validate_user_totp(current_user, user_params["code"]) do
      :valid_totp ->
        Remindly.Logs.log("totp.validate", %{user: current_user})
      ...
      {:valid_backup_code, remaining} ->
        Remindly.Logs.log("totp.validate_with_backup_code", %{user: current_user})
      ...
      :invalid ->
        Remindly.Logs.log("totp.invalid_code_used", %{user: current_user})
  ...
  defp redirect_if_totp_is_not_pending(conn, _opts) do
    if get_session(conn, @pending) do
      ...
      conn
      |> redirect(to: RemindlyWeb.Helpers.home_path(conn.assigns.current_user))
      |> halt()
    end
  end
end
