defmodule RemindlyWeb.UserAuth do
  @moduledoc """
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_remindly_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      RemindlyWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end

    if conn.assigns[:current_user] do
      Remindly.Logs.log_async("sign_out", %{user: conn.assigns.current_user})
    end
  ...
  defp disconnect_user_tokens(users_tokens, delete_too? \\ false) do
    for user_token <- users_tokens do
      RemindlyWeb.Endpoint.broadcast(user_session_topic(user_token.token), "disconnect", %{})
  ...
  def require_authenticated_user(conn, opts) do
    cond do
      ...
      true ->
        # _SETUP_TODO: this function `require_authenticated_user` is a plug that you use in your router to protect routes to only authorized users.
  ...
  defp signed_in_path(current_user), do: RemindlyWeb.Helpers.home_path(current_user)
