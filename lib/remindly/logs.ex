defmodule Remindly.Logs do
  ...
  def create(attrs \\ %{}) do
    case %Log{}
         ...
         |> Repo.insert() do
      {:ok, log} ->
        RemindlyWeb.Endpoint.broadcast("logs", "new-log", log)
        {:ok, log}
  ...
  def log_async(action, params) do
    Remindly.BackgroundTask.run(fn ->
      log(action, params)
    end)
  end
  ...
  def exists?(params) do
    ...
    |> Remindly.Repo.exists?()
  end

  def get_last_log_of_user(user) do
    Remindly.Logs.LogQuery.by_user(user.id)
