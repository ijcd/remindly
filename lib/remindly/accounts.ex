defmodule Remindly.Accounts do
  @moduledoc """
  ...
  alias Remindly.Accounts.UserNotifier
  alias Remindly.Accounts.UserToken
  alias Remindly.Accounts.UserTOTP
  ...
  def user_lifecycle_action("after_register", user, _) do
    Remindly.Logs.log_async("register", %{user: user})
    Remindly.Orgs.sync_user_invitations(user)

    Remindly.Slack.message("""
    :bust_in_silhouette: *A new user joined!*
    *Name*: #{RemindlyWeb.Helpers.user_name(user)}

    #{RemindlyWeb.Router.Helpers.admin_users_url(RemindlyWeb.Endpoint, :edit, user)}
    """)
  end

  def user_lifecycle_action("after_confirm_email", user, _) do
    Remindly.Logs.log_async("confirm_email", %{user: user})
    Remindly.Orgs.sync_user_invitations(user)
  end

  def user_lifecycle_action("after_sign_in", user, %{ip: ip}) do
    Remindly.Logs.log_async("sign_in", %{user: user})
    {:ok, user} = update_last_signed_in_info(user, ip)
    Remindly.MailBluster.sync_user_async(user)
  end

  def user_lifecycle_action("after_update_profile", user, _) do
    Remindly.Logs.log_async("update_profile", %{user: user})
    Remindly.MailBluster.sync_user_async(user)
  end

  def user_lifecycle_action("after_confirm_new_email", user, _) do
    Remindly.Logs.log_async("confirm_new_email", %{user: user})
    Remindly.Orgs.sync_user_invitations(user)
  end

  def user_lifecycle_action("request_new_email", user, %{new_email: new_email}) do
    Remindly.Logs.log_async("request_new_email", %{user: user, metadata: %{new_email: new_email}})
  end

  def user_lifecycle_action("after_passwordless_pin_sent", user, %{pin: pin}) do
    Remindly.Logs.log("passwordless_pin_sent", %{user: user})
