defmodule RemindlyWeb.PasswordlessAuthLiveTest do
  use RemindlyWeb.ConnCase
  import Phoenix.LiveViewTest
  import Remindly.AccountsFixtures
  alias Remindly.Repo

  describe "no existing user" do
    test "it creates a new user and logs them in", %{conn: conn} do
      inital_user_count = Repo.count(Remindly.Accounts.User)
      ...
      assert inital_user_count + 1 == Repo.count(Remindly.Accounts.User)
      new_user = Repo.last(Remindly.Accounts.User)
      ...
      sign_in_token =
        new_user
        |> Remindly.Accounts.generate_user_session_token()
        |> Base.encode64()

      form = form(view, "form", %{"auth" => %{"sign_in_token" => sign_in_token}})
      conn = follow_trigger_action(form, conn)
      assert redirected_to(conn) =~ RemindlyWeb.Helpers.home_path(new_user)
  ...
  describe "with existing user" do
    test "it logs in the existing user", %{conn: conn} do
      ...
      conn = follow_trigger_action(form, conn)
      assert redirected_to(conn) =~ RemindlyWeb.Helpers.home_path(user)
      assert inital_user_count == Repo.count(Remindly.Accounts.User)
    end
  end
end
