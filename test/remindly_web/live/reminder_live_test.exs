  ...
  describe "Index" do
    ...
    test "saves new reminder", %{conn: conn} do
      ...
      {:ok, _, html} =
        ...
        |> render_submit()
        |> follow_redirect(conn, ~p"/reminders")
    ...
    test "updates reminder in listing", %{conn: conn, reminder: reminder} do
      ...
      assert index_live |> element("a[href='/reminders/#{reminder.id}/edit']", "Edit") |> render_click() =~
               "Edit Reminder"
  ...
  describe "Show" do
    ...
    test "updates reminder within modal", %{conn: conn, reminder: reminder} do
      ...
      {:ok, _, html} =
        ...
        |> form("#reminder-form", reminder: @update_attrs)
        |> render_submit()
        |> follow_redirect(conn, ~p"/reminders/#{reminder}")
